...
export class DirExecuter extends CommandExecuter<DirInput> {
	...
	protected build({ path }: DirInput): CommandExe {
		const args = (new DirBuilder())
			.detailedOutput()
			.output();
		return { command: 'ls', args: args.concat(path) };
	}
	protected spawn({ command: commmand, args }: CommandExe): ChildProcessWithoutNullStreams {
		return spawn(commmand, args);
	}
	protected processStream(stream: ChildProcessWithoutNullStreams, output: StreamLogger): void {
